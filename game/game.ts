...
export const ShadowkhanGame: Game<ShadowkhanG> = {
  ...
  setup: (ctx: Ctx): ShadowkhanG => {
    const deck0 = (ctx.random as { Shuffle: <T>(a: T[]) => T[] }).Shuffle([...ALL_LABELS]);
    const deck1 = (ctx.random as { Shuffle: <T>(a: T[]) => T[] }).Shuffle([...ALL_LABELS]);
  ...
  turn: {
    ...
    moves: {
      ...
      playCard: {
        client: false,
        move: (G: ShadowkhanG, ctx: Ctx, handIndex: number, slot: number) => {
          const pid = ctx.currentPlayer;
      ...
      attackBattleCard: {
        client: false,
        move: (G: ShadowkhanG, ctx: Ctx, mySlot: number, theirSlot: number) => {
          const pid = ctx.currentPlayer;
          const opp = pid === '0' ? '1' : '0';
      ...
      attackHand: {
        client: false,
        move: (G: ShadowkhanG, ctx: Ctx, mySlot: number, theirHandIndex: number) => {
          const pid = ctx.currentPlayer;
          const opp = pid === '0' ? '1' : '0';
      ...
      attackDeck: {
        client: false,
        move: (G: ShadowkhanG, ctx: Ctx, mySlot: number) => {
          const pid = ctx.currentPlayer;
          const opp = pid === '0' ? '1' : '0';
